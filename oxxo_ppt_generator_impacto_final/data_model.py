...
def read_book(source):
    ...
    # Algunas versiones del Book no incluyen hojas auxiliares como Hoja3.
    # Cargamos las hojas disponibles y dejamos las faltantes como DataFrame
    # vacío para que la app pueda continuar usando JUN y el resto del libro.
    workbook = pd.ExcelFile(source)
    available = set(workbook.sheet_names)
    sheets = {
        s: pd.read_excel(workbook, sheet_name=s) if s in available else pd.DataFrame()
        for s in SHEETS
    }
