...
def render(fields, sheets, images):
    ...
    def build_generator_cards(group, fallback_images=False):
        ...
        for index, card in enumerate(source[:4], start=1):
            ...
            if not image_data and fallback_images:
                # Compatibilidad con JSON antiguos que usaban generator_image_N.
                image_data = images.get(f'generator_image_{index}')
            cards_out.append(
                f'<div class="generator-card">'
                f'{media(image_data, "generator-img", "Sin foto", f"Generador {index}")}'
    ...
    slides.append(slide('Tienda Hermana', f'''
        <div class="sister-layout">
            ...
            <div class="sister-name-card"><span>TIENDA HERMANA SELECCIONADA</span><strong>{text(fields.get('book_store', 'Pendiente'))}</strong><p>{text(fields.get('similar_comments', ''), 'Sin comentarios adicionales')}</p><p>{link('Ver tienda hermana', fields.get('sister_store_link'))}</p></div>
        </div>
    ''', number=8))
    ...
    if str(fields.get('microsaturation_enabled', 'No')) == 'Sí':
        ...
        micro_images = [(index, image) for index, image in enumerate(micro_images, start=1) if image]
        if micro_images:
            for index, image in micro_images:
                slides.append(slide(f'Microsaturación {index}', f'<div class="micro-grid count-1">{media(image, "micro-photo", "Foto de microsaturación", f"Microsaturación {index}")}</div>', number=None))
        else:
            slides.append(slide('Microsaturación', '<div class="micro-grid count-1">' + media(None, 'micro-photo', 'Sube hasta 5 fotos de microsaturación', 'Microsaturación') + '</div>', number=None))
...
.image-placeholder { display:flex; align-items:center; justify-content:center; min-height:1in; color:#9A8D7D; background:repeating-linear-gradient(135deg,#F4F0E8 0,#F4F0E8 10px,#EEE8DD 10px,#EEE8DD 20px); font-size:8pt; font-weight:800; letter-spacing:.06em; text-transform:uppercase; }
    ...
    .micro-grid.count-1 { grid-template-columns:1fr; grid-auto-rows:5.7in; }
