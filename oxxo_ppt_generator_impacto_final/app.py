...
with st.expander('Tienda Hermana'):
    ...
    f['sister_store_link'] = st.text_input('Link de tienda hermana', f.get('sister_store_link', ''), key='s8_link')
...
if generate:
    ...
    micro_count = sum(bool(image_bytes.get(f'microsaturation_image_{i}')) for i in range(1, 6))
    slide_count = 12 + micro_count
